...
impl<'a, 'ctx> AsyncToGenerator<'a, 'ctx> {
    ...
    fn transform_function_declaration(
        ...
        // function _name() { _ref.apply(this, arguments); }
        {
            ...
            statements.push(self.create_async_to_generator_assignment(
                bound_ident.create_write_reference(ctx),
                params,
                body,
                generator_scope_id,
                ctx,
    ...
    fn infer_function_id_from_variable_declarator(
        ...
        function_name.map(|name| {
            ...
            ctx.scopes_mut().add_binding(scope_id, name.to_compact_str(), symbol_id);
            ctx.ast.binding_identifier_with_symbol_id(SPAN, name.clone(), symbol_id)
        })
    }
